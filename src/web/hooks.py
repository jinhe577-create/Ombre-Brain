    ...
    async def breath_hook(request):
        ...
        try:
            async with _timeout_after(timeout_seconds):
                ...
                # 原文展示不脱水（不占 dehydrate 配额），走与其他记忆一致的
                # 防注入数据块封装。
                try:
                    _passages = [
                        b for b in all_buckets
                        if "__passage__" in ((b.get("metadata") or {}).get("domain") or [])
                    ]
                    if _passages:
                        _passages.sort(
                            key=lambda b: (str(b["metadata"].get("created", "")), str(b["id"])),
                            reverse=True,
                        )
                        _lp = _passages[0]
                        _ptext = strip_wikilinks(str(_lp.get("content") or ""))
                        _pdate = str(_lp["metadata"].get("created", ""))[:10]
                        if _ptext:
                            append_block(_hook_data_block(
                                _lp,
                                f"🪟 [上个窗口给你的留言 · {_pdate}]\n{_ptext}",
                                role="passage_message",
                            ))
                except Exception as exc:
                    logger.warning("breath_hook passage section failed: %s", exc)
